...
def off_train_DQN(policy, jobsList, machinesList, maxIter, dispatchingRules):
    method_DQN = DQN(policy)
    ...
    for epoch in range(maxIter):
        ...
        while len(resultSPT) != len(jobsListCopy):
            ...
            action = method_DQN.choose_action(features, adjacent_matrix)
            ...
            method_DQN.store_transition(state, action, reward, next_state_Infor, done)
            if method_DQN.memory_counter > MEMORY_CAPACITY:
                loss = method_DQN.learn()
                # losses.append(loss)
            state = state_
            # total_reward += reward
