...
class GA(torch.nn.Module):
    ...
    def GA_trainer(self, b_s_):
        ...
        for machine in self.idel_machine:
            ...
            # 如果该空闲机器的可执行队列中无待执行操作，则该机器不参与后续操作
                    ...
                    if not mac_que[index].completed:
                        ...
                        if len(mac_que) == 1:
                            ...
                            if len(no_processed_job) > 0:
                                ...
                                X[i, j] = index[0]     # candidate_queue[idel_machine[j].name][index]为分配给该机器的job
                                mac_que[int(X[i, j])].completed = True
                            else:
                                X[i, j] = -1
            # 开始迭代
            ...
            while t <= self.maxFEs:
                ...
                for i in range(self.N):
                    if random.random() <= self.pm:
                        ...
                        # mac_queue = self.candidate_queue[mac.name]
        ...
        # plt.title('iter')
        # plt.plot(X, corve, linewidth=2)
        # figure_name = 'E:/Phd_work/myWork/code/Model/bets_fit.png'
    ...
    def forward(self, b_s_):
        ...
        return reward,resultToExpert,jobsList, machinesList,pro_time    # 由于是经过多次迭代之后的结果，相当于将agent不断的放在该状态下进行游戏获得的最好的值，将其视为收敛后的Q_target

class QNGA(object):
    # 以policy作为评估策略，GA作为目标策略
    def __init__(self, policy, GA_selection):  # 定义DQN的一系列属性
        ...
        self.BATCH_SIZE = 64
        self.optimizer = torch.optim.Adam(self.eval_net.parameters(), lr=1e-5  )  # 使用Adam优化器 (输入为评估网络的参数和学习率)
        self.loss_func = nn.MSELoss()  # 使用均方损失函数 (loss(xi, yi)=(xi-yi)^2)
        self.memory = deque(maxlen=10000)
        self.GA_memory = deque(maxlen=10000)
        self.N_STATES = 7
        self.EPSILON = 0.9  # greedy policy
        self.E_greedy_decrement = 0.001  # 随着训练逐步收敛，探索的程度慢慢降低
    ...
    def choose_action(self, features, adjacent_matrix):
        ...
        self.EPSILON = max(0.01, self.EPSILON - self.E_greedy_decrement)  # 随着训练逐步收敛，探索的程度慢慢降低
        return action
...
def off_train_QNGA(policy, jobsList, machinesList, maxIter):
    ...
    # plot loss_graph
    figure_name = 'E:\Phd_work\myWork\code\Model\loss_figure'
    X = range(len(makespan))
    plt.figure(figsize=(8, 6))
    plt.xlabel("epoch")
    plt.ylabel("makespan")
    plt.rcParams['axes.unicode_minus'] = False  # 用来正常显示负号
    # plt.xticks(X, labels='epoch', rotation=45)
    plt.title('training errors')
    plt.plot(X, makespan, linewidth=2)
    plt.savefig(figure_name)
...
if __name__ == '__main__':
    ...
    # policy = Policy()
    # 从上次保存的模型开始训练
    # model_path = 'E:/Phd_work/myWork/code/Model/best_model/QNGA'
    model_path = 'E:\Phd_work\myWork\code\Model\QNGA_model_2022_11_10_15_14_50\QNGA'
    policy = torch.load(model_path)

    dispatchingRules = ['algorithmSPT', 'algorithmLPT', 'algorithmFOPNR', 'algorithmMORPNR', 'algorithmSR',
                        'algorithmLR', 'randomSolution']

    dataPath = 'E:\Phd_work\myWork\code\TextData\Monaldo\Fjsp\Job_Data\Brandimarte_Data\Text\Mk01.fjs'
    jobList, machineList, nJob = readData(dataPath)
    policy, makespan = off_train_QNGA(policy, jobList, machineList, maxIter)
    now_time = time.localtime()
    subPathName = str(now_time.tm_year) + '_' + str(now_time.tm_mon) + '_' + str(now_time.tm_mday) + '_' + str(
        now_time.tm_hour) + '_' + str(now_time.tm_min) + '_' + str(now_time.tm_sec)
    model_path_root = 'E:\Phd_work\myWork\code\Model'
    sub_model_path = model_path_root + '/QNGA_model_' + subPathName
    os.makedirs(sub_model_path)
    model_path = sub_model_path + '/QNGA'
    torch.save(policy, model_path)

    # 画makespan_epoch
    figure_name = sub_model_path + '/makespan_figure.png'
    X = range(len(makespan))
    plt.figure(figsize=(8, 6))
    plt.xlabel("epoch")
    plt.ylabel("makespan")
    ...
    # 记录epoch-makespan的值
    file_name = sub_model_path + '/makeSpan.xlsx'
    xls = xlwt.Workbook()
    sht1 = xls.add_sheet('makespan')
    sht1.write(0, 0, 'epoch')
    sht1.write(0, 1, 'makespan')
    epoch = 0
    for item in makespan:
        epoch += 1
        sht1.write(epoch, 0, epoch)
        sht1.write(epoch, 1, item)
    xls.save(file_name)